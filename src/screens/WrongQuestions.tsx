...
export function WrongQuestions() {
  ...
  const rows = useMemo(() => {
    ...
    return [...lastWrong.entries()]
      ...
      .filter((r) => (!cat || r.q.category === cat) && (!sub || r.q.subtopic === sub) && r.at >= since && (!onlyPending || !r.h.lastCorrect))
      .sort((a, b) => b.at - a.at);
  }, [store.answers, store.qById, store.history, cat, sub, period, onlyPending]);

  const subs = useMemo(() => [...new Set(store.answers.filter((a) => !a.correct && (!cat || a.cat === cat)).map((a) => a.sub))].sort(), [store.answers, cat]);

  return (
    <div className="pb-10">
      <Header title="Meus cartões errados" subtitle={`${rows.length} cartão(ões)`} />
      ...
      {rows.length > 0 && (
        <Btn big variant="danger" className="w-full mt-3" onClick={() => store.nav({ name: 'trainingRun', config: { count: rows.length, categories: [], wrongOnly: false, questionIds: rows.map((r) => r.q.id) } })}>
          🔁 REFAZER CARTÕES ERRADOS ({rows.length})
        </Btn>
      )}

      {rows.length === 0 ? (
        <Empty icon="🎯" text="Nenhum cartão errado com esses filtros." />
      ) : (
          ...
          {rows.map(({ q, at, h }) => (
            <Card key={q.id} className="p-3 cursor-pointer hover:border-white/25 transition" onClick={() => setOpen(q)}>
              ...
              <div className="flex items-center gap-3 mt-2 text-[11px]">
                ...
                <span className={h.lastCorrect ? 'text-emerald-300' : 'text-amber-300'}>{h.lastCorrect ? 'recuperado' : 'pendente'}</span>
